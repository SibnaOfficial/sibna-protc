...
/// Sanitize a byte array by removing null bytes.
///
/// # WARNING
/// This function is intended ONLY for **human-readable strings** (e.g. log fields).
/// Do NOT use it on binary protocol data — removing null bytes silently corrupts
...
///
/// Use this for **binary validation**: reject the input rather than silently
/// mutating it (see `sanitize_bytes` warning above).
pub fn contains_null_byte(data: &[u8]) -> bool {
    data.contains(&0)
}
