...
// ── Domain separator for group message signing ───────────────────────────────
static constexpr const char* GROUP_MSG_DOMAIN = "SibnaGroupMessage_v3";

// ── GroupMessage ─────────────────────────────────────────────────────────────

bytes GroupMessage::signable_bytes() const {
    bytes result;
    result.reserve(32 + 4 + 4 + ciphertext.size() + 8 + 24);

    // Domain separator
    result.insert(result.end(), GROUP_MSG_DOMAIN, GROUP_MSG_DOMAIN + 24);

    // Group ID
    result.insert(result.end(), group_id_.begin(), group_id_.end());

    // Sender key ID (4 bytes LE)
    result.push_back(static_cast<byte>(sender_key_id & 0xFF));
    result.push_back(static_cast<byte>((sender_key_id >> 8) & 0xFF));
    result.push_back(static_cast<byte>((sender_key_id >> 16) & 0xFF));
    result.push_back(static_cast<byte>((sender_key_id >> 24) & 0xFF));

    // Message number (4 bytes LE)
    result.push_back(static_cast<byte>(message_number & 0xFF));
    result.push_back(static_cast<byte>((message_number >> 8) & 0xFF));
    result.push_back(static_cast<byte>((message_number >> 16) & 0xFF));
    result.push_back(static_cast<byte>((message_number >> 24) & 0xFF));

    // Ciphertext
    result.insert(result.end(), ciphertext.begin(), ciphertext.end());

    // Epoch (4 bytes LE)
    result.push_back(static_cast<byte>(epoch & 0xFF));
    result.push_back(static_cast<byte>((epoch >> 8) & 0xFF));
    result.push_back(static_cast<byte>((epoch >> 16) & 0xFF));
    result.push_back(static_cast<byte>((epoch >> 24) & 0xFF));
...
Result<void> GroupMessage::sign(const key& ed25519_private_key) {
    bytes sb = signable_bytes();
    auto sig_result = Ed25519::sign(ed25519_private_key, sb);
    if (sig_result.is_err()) {
        return Result<void>(sig_result.code(), sig_result.message());
    }
    msg_signature = sig_result.value();
    return Result<void>();
}

Result<bool> GroupMessage::verify(const key& ed25519_public_key) const {
    bytes sb = signable_bytes();
    return Ed25519::verify(ed25519_public_key, sb, msg_signature);
}

bytes GroupMessage::to_bytes() const {
    bytes result;
    result.reserve(32 + 4 + 4 + ciphertext.size() + 4 + 8 + 64);
    ...
    result.push_back(static_cast<byte>(ct_len & 0xFF));
    result.push_back(static_cast<byte>((ct_len >> 8) & 0xFF));
    result.push_back(static_cast<byte>((ct_len >> 16) & 0xFF));
    result.push_back(static_cast<byte>((ct_len >> 24) & 0xFF));
    ...
    // Signature (64 bytes)
    result.insert(result.end(), msg_signature.begin(), msg_signature.end());

    return result;
}

Result<GroupMessage> GroupMessage::from_bytes(const bytes& data) {
    // Minimum: 32 + 4 + 4 + 4 + 4 + 8 + 64 = 120
    if (data.size() < 32 + 4 + 4 + 4 + 4 + 8 + 64) {
    ...
    // Sender key ID
    msg.sender_key_id = static_cast<uint32_t>(data[offset])
    ...
    // Message number
    msg.message_number = static_cast<uint32_t>(data[offset])
                       | (static_cast<uint32_t>(data[offset + 1]) << 8)
                       | (static_cast<uint32_t>(data[offset + 2]) << 16)
                       | (static_cast<uint32_t>(data[offset + 3]) << 24);
    offset += 4;
    ...
    // Ciphertext length
    uint32_t ciphertext_len = static_cast<uint32_t>(data[offset])
                            | (static_cast<uint32_t>(data[offset + 1]) << 8)
                            | (static_cast<uint32_t>(data[offset + 2]) << 16)
                            | (static_cast<uint32_t>(data[offset + 3]) << 24);
    offset += 4;

    if (data.size() < offset + ciphertext_len + 4 + 8 + 64) {
    ...
    // Epoch
    msg.epoch = static_cast<uint32_t>(data[offset])
              | (static_cast<uint32_t>(data[offset + 1]) << 8)
              | (static_cast<uint32_t>(data[offset + 2]) << 16)
              | (static_cast<uint32_t>(data[offset + 3]) << 24);
    offset += 4;
    ...
    // Signature
    std::copy(data.begin() + offset, data.begin() + offset + 64, msg.msg_signature.begin());
